...
#start here 
    ...
    #print max_range
    square_mid_points=square_location(max_x, max_y, max_z , max_range); #for hybrid-circle
   # print 'square mid points', len(square_mid_points)
...
#                                     #np.random.uniform(0.8 * max_z, 1 * max_z)];
#                                     np.random.uniform(-0.8 * max_z, -1 * max_z)];
        track_nodes.append(nodes[node].x_y_zreal);
    #dist_inter_mean2=np.mean(dist_inter_2)
#    dist_inter_mean2=np.mean(dist_inter_2)
#    dist_inter_mean3=np.mean(dist_inter_3)
##
#    #dist_inter_std=np.std(dist_inter)
##    test=np.sum(dist_inter)/len(dist_inter)
    #print 'mean dist',dist_inter_mean2
#    , dist_inter_mean2,dist_inter_mean3
 ...
 #   dist_inter = []
   ...
   # dist_inter_mean=np.mean(dist_inter)
    #dist_inter_std=np.std(dist_inter)
#    test=np.sum(dist_inter)/len(dist_inter)
   # print 'mean dist', dist_inter_mean
    #print 'bla', nodes[0].x_y_zreal 
        ...
        for uav in range(len(uavs)):
            ...
            if (len(localised_nodes) > 0):
               ...
               #for this we use a global variable shai 
                ...
                if (distance.euclidean(uavs[uav].uav_current_coord, nodes[localised_nodes[i][0]].x_y_zestim) < step/18 ):   #### main distance between uav and node,for it to form static structure 
                    if i + 1 < len(localised_nodes): ##main1           
                         ...
                         temp = g_shaped(uavs[uav].uav_current_coord, step);
              ...
              # we don't have any visible nodes with known unlocalized neighboard, do random walk
                ...
                if(shai==0):## Normal flow of program
                    uavs[uav].uav_current_coord,square_mid_points = next_location_and_updated_database([uavs[uav].uav_current_coord],square_mid_points)
                    uavs[uav].uav_current_coord = random_walk(uavs[uav].uav_current_coord, max_x, max_y, max_z, step);               
            if(shai==5): #as when 26 extra points are  be created, around the localised node with many unlocalised neighbours,
