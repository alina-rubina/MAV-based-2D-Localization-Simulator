...
def random_walk(x_y_zinit_qc,max_x,max_y,max_z,step): 
    step=step/3
    max_x=abs(max_x); 
    max_y=abs(max_y);
    max_z=abs(max_z);
...
def circle_walk(x_y_zinit_qc,max_x,max_y,max_z,step,i): 
    max_x=abs(max_x); 
    max_y=abs(max_y);
    max_z=abs(max_z);
    random.seed(None); # Seed generator, None = system clock
    ...
    #theta = i*2*math.pi/3 + add
    theta = math.pi/16*i #+ add
    #math.acos(step/(2*45))*i#*math.pi*i # 0 =< angle =< 2 pi
    #theta = 0.125*math.pi#math.acos(step/(2*45))*i
    x[1] = x[0] + step*cos(theta) #-1 =< x =< 1
...
#
#coord = []
#coord = circle_walk(x_init_qc,y_init_qc,max_x,max_y,0,48)
#
#   
